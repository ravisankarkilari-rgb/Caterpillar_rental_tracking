...
from backend.models import Equipment, Alert, AlertType, AlertSeverity, EquipmentStatus, UsageLog
from backend.services.equipment_service import calculate_equipment_metrics, get_all_equipment_metrics
from backend.services.anomaly_service import detect_anomalies_for_equipment

def sync_alerts_for_equipment(
    equipment: Equipment,
    db: Session,
    precalculated_metrics: dict = None,
    precalculated_logs: list = None,
    precalculated_alerts: dict = None
):
    """
    ...
    """
    today = date.today()
    metrics = precalculated_metrics if precalculated_metrics is not None else calculate_equipment_metrics(equipment, db)
    
    # Active alerts map for this equipment: alert_type -> Alert object
    existing_alerts_map = precalculated_alerts if precalculated_alerts is not None else {}
    
    def get_active_alert(alert_type: str):
        if precalculated_alerts is not None:
            return existing_alerts_map.get(alert_type)
    ...
    if equipment.status == EquipmentStatus.AVAILABLE.value:
        ...
        if metrics["days_overdue"] > 0:
            existing_overdue = get_active_alert(AlertType.OVERDUE_RENTAL.value)
        ...
        # Check Return Due Soon
            ...
            existing_due_soon = get_active_alert(AlertType.RETURN_DUE_SOON.value)
        ...
        if metrics["operating_days"] > 0 and metrics["utilization_percentage"] < 30.0:
            existing_under_util = get_active_alert(AlertType.UNDER_UTILIZED.value)
    ...
    anomalies = detect_anomalies_for_equipment(equipment, db, precalculated_logs=precalculated_logs)
    for anom in anomalies:
        existing_anom = get_active_alert(anom["alert_type"])
...
def sync_all_alerts(db: Session):
    ...
    # Bulk query 1: All equipment metrics in 2 SQL queries
    metrics_map = get_all_equipment_metrics(equipments, db)

    # Bulk query 2: All usage logs grouped by equipment_id in 1 SQL query
    all_logs = db.query(UsageLog).order_by(UsageLog.date.desc()).all()
    logs_by_id = defaultdict(list)
    for log in all_logs:
        logs_by_id[log.equipment_id].append(log)

    # Bulk query 3: All active alerts grouped by equipment_id & alert_type in 1 SQL query
    active_alerts = db.query(Alert).filter(Alert.resolved == False).all()
    alerts_by_id = defaultdict(dict)
    for alt in active_alerts:
        alerts_by_id[alt.equipment_id][alt.alert_type] = alt

    for equip in equipments:
        sync_alerts_for_equipment(
            equip,
            db,
            precalculated_metrics=metrics_map.get(equip.equipment_id, {}),
            precalculated_logs=logs_by_id.get(equip.equipment_id, []),
            precalculated_alerts=alerts_by_id.get(equip.equipment_id, {})
        )
